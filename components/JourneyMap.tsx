...
export const JourneyMap: React.FC = () => {
    ...
    const handleBigFiveAnswer = (score: number) => {
        const newAnswers = { ...bigFiveAnswers, [BIG_FIVE_QUESTIONS[bigFiveStep].id]: score };
        setBigFiveAnswers(newAnswers);
        if (bigFiveStep < BIG_FIVE_QUESTIONS.length - 1) {
            setBigFiveStep(prev => prev + 1);
        } else {
            // Calculate result
            const traits: Record<string, number> = {};
            Object.entries(newAnswers).forEach(([qId, s]) => {
                const q = BIG_FIVE_QUESTIONS.find(x => x.id === Number(qId));
                if (q) {
                    traits[q.trait] = (traits[q.trait] || 0) + (s as number);
                }
            });

            let maxTrait = '';
            let maxScore = -1;
            Object.entries(traits).forEach(([trait, s]) => {
                if (s > maxScore) {
                    maxScore = s;
                    maxTrait = trait;
                }
            ...
            if (updateUser) {
                updateUser({ bigFiveTrait: maxTrait });
            }
            ...
            <div className="max-w-md mx-auto relative pt-12 px-4 flex flex-col items-center gap-20 pb-32">
                ...
                {phases.map((phase, index) => {
                    ...
                    `}
                                ...
                                {/* Label */}
                                <div className={`mt-5 px-5 py-2 rounded-2xl border-2 border-b-4 transition-all duration-300 ${isActive
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none">
                    ...
                    <div className="bg-[rgb(var(--color-snow))] w-full max-w-md mx-auto rounded-t-[2rem] sm:rounded-[2rem] p-8 pointer-events-auto animate-slide-up shadow-2xl relative overflow-hidden border-2 border-[rgb(var(--color-swan))]">
                        ...
                        <div className="relative z-10 flex flex-col items-center text-center -mt-4">
                            <div className={`w-24 h-24 rounded-3xl shadow-lg flex items-center justify-center mb-6 border-4 border-white ${selectedPhase.status === 'locked' ? 'bg-[rgb(var(--color-swan))] text-[rgb(var(--color-hare))]' : `${selectedPhase.color} text-white`
                                }`}>
                            ...
                            <div className="space-y-3">
                                {[1, 2, 3, 4, 5].map((score) => (
                                    <button
                                        ...
                                        className="w-full text-left p-4 rounded-2xl border-2 border-[rgb(var(--color-swan))] border-b-4 hover:bg-[rgb(var(--color-polar))] hover:border-[rgb(var(--color-macaw))] transition-all font-bold text-[rgb(var(--color-wolf))] hover:text-[rgb(var(--color-macaw))] flex items-center justify-between group active:border-b-0 active:translate-y-1"
                                    >
                                        ...
                                        <div className="w-6 h-6 rounded-full border-2 border-[rgb(var(--color-swan))] group-hover:border-[rgb(var(--color-macaw))] flex items-center justify-center flex-shrink-0 ml-4">
