...
import { Home, BookOpen, User as UserIcon, Calendar, X, Map, Shield } from 'lucide-react';
...
const adminNavItems: NavItem[] = [
  ...navItems,
  { label: 'Painel Admin', path: '/admin', icon: Shield },
];
...
const SidebarContent: React.FC<{ onClose?: () => void }> = ({ onClose }) => {
  const { user } = useAuth();
  const itemsToRender = user?.isAdmin ? adminNavItems : navItems;
      ...
      <nav className="space-y-2">
        {itemsToRender.map((item) => (
          <NavLink
            ...
            onClick={onClose}
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3.5 text-sm font-bold rounded-2xl transition-all duration-200 group relative border-2 border-transparent ${isActive
